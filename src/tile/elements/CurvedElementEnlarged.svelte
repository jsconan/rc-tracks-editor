<script>
     ...
     * @private
     */
    function curvedElementPath(innerRadius, curveWidth, curveSide, centerX, centerY) {
        const outerRadius = innerRadius + curveWidth - curveSide;
        const center = new Vector2D(centerX, centerY);

        const p1 = center.addScalarX(innerRadius);
        const p2 = center.addScalarY(innerRadius);
        const p3 = p2.addScalarY(curveWidth);
        const p4 = p3.addScalarX(curveSide);
        const p5 = p4.addCoord(outerRadius, -outerRadius);
        const p6 = p5.subScalarY(curveSide);
