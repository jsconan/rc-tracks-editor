...
describe('StraightTileModel', () => {
    ...
    describe('can build a tile', () => {
        it('with the given size', () => {
            const tile = new StraightTileModel(laneWidth, barrierWidth, barrierChunks);
        ...
        it('with the given size and ratio', () => {
            const tile = new StraightTileModel(laneWidth, barrierWidth, barrierChunks, 3);
        ...
        it('of the expected type', () => {
            ...
            expect(tile.getType()).toBe(StraightTileModel.TYPE);
        });
    });
    ...
    describe('can compute', () => {
        ...
        describe('the rotation angle for a tile', () => {
            ...
            it.each(tileRatios)('oriented to the right with a ratio of %s', ratio => {
                const tile = new StraightTileModel(laneWidth, barrierWidth, barrierChunks, ratio);
                expect(tile.getDirectionAngle(StraightTileModel.DIRECTION_RIGHT)).toMatchSnapshot();
                expect(tile.getDirectionAngleRight()).toMatchSnapshot();
            });

            it.each(tileRatios)('oriented to the left with a ratio of %s', ratio => {
                const tile = new StraightTileModel(laneWidth, barrierWidth, barrierChunks, ratio);
                expect(tile.getDirectionAngle(StraightTileModel.DIRECTION_LEFT)).toMatchSnapshot();
        ...
        describe('the position of the output point for a tile', () => {
            ...
            it.each(tileRatios)('oriented to the right with a ratio of %s', ratio => {
                ...
                expect(tile.getOutputCoord(StraightTileModel.DIRECTION_RIGHT)).toMatchSnapshot();
                expect(tile.getOutputCoord(StraightTileModel.DIRECTION_RIGHT, tileX, tileY)).toMatchSnapshot();
                expect(tile.getOutputCoord(StraightTileModel.DIRECTION_RIGHT, tileX, tileY, 90)).toMatchSnapshot();
    ...
    describe('has a static property', () => {
        describe('getStraightTileModelWidth', () => {
            it('which computes the width of a tile knowing the width of its lane and its barrier', () => {
                expect(StraightTileModel.getTileWidth(laneWidth, barrierWidth)).toBe(tileWidth);
            });
        });

        describe('getStraightTileModelLength', () => {
            it('which computes the length of a tile knowing the width of its lane and its barrier', () => {
                expect(StraightTileModel.getTileLength(laneWidth, barrierWidth)).toBe(tileLength);
            });
        });

        describe('getLaneWidth', () => {
            it('which computes the width of a lane knowing the width of a tile and its barrier', () => {
                expect(StraightTileModel.getLaneWidth(tileWidth, barrierWidth)).toBe(laneWidth);
            });
        });

        describe('getCurveAngle', () => {
            it('which computes the angle of the curve with respect of the size ratio', () => {
                expect(StraightTileModel.getCurveAngle()).toBe(90);
                expect(StraightTileModel.getCurveAngle(0.5)).toBe(45);
                expect(StraightTileModel.getCurveAngle(1)).toBe(90);
                expect(StraightTileModel.getCurveAngle(2)).toBe(45);
                expect(StraightTileModel.getCurveAngle(3)).toBe(30);
                expect(StraightTileModel.getCurveAngle(4)).toBe(22.5);
            });
        });
    });
});
